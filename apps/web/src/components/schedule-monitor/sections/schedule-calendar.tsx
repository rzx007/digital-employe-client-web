...
import type { MonthlyOverview } from "@/types/schedule-monitor"

function getLevel(total: number): 0 | 1 | 2 | 3 {
  if (total === 0) return 0
  if (total <= 2) return 1
  if (total <= 5) return 2
  return 3
...
export function ScheduleCalendar({
  ...
  const dayMap = useMemo(() => {
    const map = new Map<string, MonthlyOverview["days"][number]>()
    ...
    <div className="rounded-lg border p-4">
      ...
      <div className="relative grid grid-cols-7 justify-items-center gap-x-1 gap-y-1">
        {cells.map((day, i) => {
          ...
          const level = dayData ? getLevel(dayData.total) : 0
          const isToday = dateStr === todayStr
          const isFuture = dayData && dayData.date > todayStr
          ...
          return (
            <button
              ...
              className={cn(
                ...
                dayData && !isFuture && "cursor-pointer hover:opacity-80"
              )}
              onMouseEnter={(e) => {
                if (!dayData || isFuture) return
                const rect = e.currentTarget.getBoundingClientRect()
                setTooltipInfo({
                  text: `${dateStr}: ${dayData.total} 个任务 (${dayData.success} 完成${dayData.failed > 0 ? `, ${dayData.failed} 失败` : ""})`,
