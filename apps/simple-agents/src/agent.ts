 ...
 * 共享 Checkpointer 实例
 *
 * 所有 Agent 实例共享同一个 checkpointer，通过 thread_id（sessionId）隔离不同会话的状态。
 * 使用 MemorySaver 实现，重启后状态丢失。
 * 生产环境建议换用 @langchain/langgraph-checkpoint-sqlite。
 */
...
/**
 * Question 工具 — Agent 向用户提问并等待回答
 *
 * 当 Agent 缺少必要信息时，调用此工具暂停执行，等待用户输入。
 * 配合 interruptOn 配置实现人机协作（Human-in-the-Loop）。
 */
const questionTool = tool(
  ({ answer }: { question: string; answer: string }) => answer,
  {
    name: "question",
    description:
      "向用户提问。当你缺少关键信息、无法独立完成任务时使用。" +
      "必须先调用此工具获取用户回答后，才能继续后续操作。" +
      "不要自己编造答案，answer 参数留空即可。",
    schema: z.object({
      question: z.string().describe("要问用户的问题"),
      answer: z.string().describe("用户的回答（由前端收集后填入）"),
    }),
  }
)
...
export function getAgent(employeeId: string, systemPrompt?: string): any {
  ...
  const agent = createDeepAgent({
    ...
    // 状态持久化：通过 thread_id（sessionId）隔离不同会话
    checkpointer,
    // 人机协作：这些工具执行前需要用户审批
    interruptOn: {
      question: true,
