...
export function getAgent(employeeId: string, systemPrompt?: string): any {
  ...
  const agent = createDeepAgent({
    ...
    backend: (runtime) => {
      ...
      return new FilesystemBackend({ rootDir: getSessionDir(threadId), virtualMode: true })
    },
    // ⚠️ 未配置 subagents
  })
...
function buildOrchestratorPrompt(groupName: string, employees: Array<{
  name: string
  description: string
...
export async function getGroupAgent(groupId: string, threadId: string): Promise<any> {
  ...
  const employeesInfo: Array<{ name: string; description: string; capabilities: any[] }> = []
...
app.post("/:id/chat/stream", async (c) => {
  ...
  if (metadata.groupId) {
    // 群聊模式：使用 GroupAgent
    return await groupChatStreamResponse(
      id,
      body.message,
      metadata.groupId
    )
...
export async function roundRobinChatStreamResponse(
  ...
  for (const empId of employeeIds) {
    ...
    // 保存每个员工的回复
    await createMessage(sessionId, "assistant", results[results.length - 1].content, {
      parts: [{ type: "text", text: results[results.length - 1].content }],
      metadata: JSON.stringify({ employeeId: empId, employeeName: emp?.name }),
    })
  }
